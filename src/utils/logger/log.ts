...
/**
 * Creates a .log file for this session, but also symlinks ${APP_NAME}-latest.log
 * to the current log file so you can reliably run:
 *
 * - Mac/Windows: `tail -F "$TMPDIR/${APP_NAME}/${APP_NAME}-latest.log"`
 * - Linux: `tail -F ~/.local/${APP_NAME}/${APP_NAME}-latest.log`
 */
export function initLogger(): Logger {
  ...
  const logDir =
    isMac || isWin
      ...
      : path.join(os.homedir(), ".local", APP_NAME);
  fsSync.mkdirSync(logDir, { recursive: true });
  const logFile = path.join(logDir, `${APP_NAME}-${now()}.log`);
  // Write the empty string so the file exists and can be tail'd.
  fsSync.writeFileSync(logFile, "");

  if (!isWin) {
    const latestLink = path.join(logDir, `${APP_NAME}-latest.log`);
    try {
