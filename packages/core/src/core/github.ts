...
import type { CoreFileSystemInterface } from "./files";
...
export async function getRepositoryInfo(
  ...
  try {
    ...
    if ((error as GitHubErrorResponse).status === 404) {
      throw ErrorFactory.github(
...
export async function validateWebhookSignature(
  ...
  try {
    ...
    if ((error as { code?: string }).code === ERROR_CODES.GITHUB_PERMISSION_DENIED) {
      throw error;
    }
    throw ErrorFactory.github(
      ERROR_CODES.GITHUB_API_ERROR,
