...
import { type FilesClaudeContextStatus, ensureClaudeContext } from "./files";
...
export interface WorktreeResult {
  ...
  contextStatus?: FilesClaudeContextStatus;
}
...
export interface ExtendedWorktreeInfo extends WorktreeInfo {
  ...
  claudeContext?: FilesClaudeContextStatus;
...
interface FileOperationsInterface {
  ensureClaudeContext(targetPath: string, sourcePath?: string): Promise<FilesClaudeContextStatus>;
}

class DefaultFileOperations implements FileOperationsInterface {
  async ensureClaudeContext(
    targetPath: string,
    sourcePath?: string,
  ): Promise<FilesClaudeContextStatus> {
...
export async function createWorktree(
  ...
  try {
    ...
    let contextStatus: FilesClaudeContextStatus | undefined;
    if (setupContext) {
      try {
